...
export default function CustomerNotifications() {
  ...
  const resolveLink = (item: any) => {
    ...
    return item.data.link || item.data.path || item.data.href || (item.data.productSlug ? `/products/${item.data.productSlug}#qa` : undefined);
  };
        ...
        <div className="space-y-3">
          {notifications.map((item: any) => {
            ...
            return (
              <div
                key={item.id}
                className={`card p-4 transition ${!item.isRead ? 'border-primary-200 bg-primary-50/30' : 'hover:shadow-lg'} ${link ? 'cursor-pointer' : ''}`}
                onClick={() => {
                  if (link) {
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    {item.data?.productSlug && (
                      <p className="mt-2 text-xs text-gray-500">Product: {item.data.productTitle || item.data.productSlug}</p>
                    )}
                    <p className="mt-2 text-xs text-gray-400">{new Date(item.createdAt).toLocaleString()}</p>
                  </div>
                  <div className="flex gap-1 items-start">
                    {!item.isRead && (
                      <button onClick={(e) => { e.stopPropagation(); markRead.mutate(item.id); }} className="p-2 text-gray-500 hover:text-primary-600" title="Mark read">
                        <Check className="h-4 w-4" />
                      </button>
                    )}
                    <button onClick={(e) => { e.stopPropagation(); remove.mutate(item.id); }} className="p-2 text-gray-500 hover:text-red-600" title="Delete">
                      <Trash2 className="h-4 w-4" />
                    </button>
                  </div>
                </div>
                {link && (
                  <div className="mt-3 text-xs text-primary-600 font-medium">Tap to view</div>
                )}
              </div>
            );
          })}
